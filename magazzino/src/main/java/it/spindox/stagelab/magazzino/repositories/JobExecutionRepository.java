...
public interface JobExecutionRepository extends JpaRepository<JobExecution, Long> {
    ...
    /**
     * Recupera l'ultima esecuzione (per startTime)
     */
    Optional<JobExecution> findFirstByOrderByStartTimeDesc();

    /**
     * Recupera un job attualmente in RUNNING
     * (utile per evitare doppie esecuzioni concorrenti)
     */
    @Query("""
        SELECT j
        FROM JobExecution j
        WHERE j.status = it.spindox.stagelab.magazzino.entities.StatusJob.RUNNING
    """)
